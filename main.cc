...
static int do_create(const char* path, char fill_char, off_t size) {
  ...
  if (size > 0) {
    void* addr = proj3::mmap(nullptr,
                             ...
                             0);
    std::memset(addr,
                static_cast<unsigned char>(fill_char),
                static_cast<std::size_t>(size));
...
static int do_insert(const char* path, off_t offset, off_t bytes_incoming) {
  ...
  if (orig_size > 0) {
    ...
    void* snap_addr = proj3::mmap(nullptr,
                                  ...
                                  fd,
                                  0);
    std::memcpy(snapshot.data(), snap_addr, static_cast<std::size_t>(orig_size));
    ...
    try {
      ...
      if (orig_size > 0) {
        void* addr = proj3::mmap(nullptr,
                                 ...
                                 fd,
                                 0);
        std::memcpy(addr, snapshot.data(), static_cast<std::size_t>(orig_size));
        proj3::msync(addr, static_cast<std::size_t>(orig_size), proj3::MS_SYNC);
...
static int do_append(const char* path, off_t bytes_incoming) {
  ...
  while (remaining > 0) {
    ...
    off_t chunk_size = current_size == 0 ? 1 : current_size;
